...
L.trans.rna <- tans.points(sc.rna.mu.scale, lam = 1, prob = 0.2)
...
L.trans.atac <- tans.points(sc.atac.mu.scale, lam = 1, prob = 0)
...
sc.rna.mu.scale <- sc.rna.mu.scale %>% 
  mutate(cluster.rna = case_when(peak.ord <  L.trans.rna$transition.points$y[1] ~ "RC1",
                                 ...
                                 peak.ord >= L.trans.rna$transition.points$y[3]  ~ "RC4"),
...
sc.rna.mu.scale <- left_join(sc.rna.mu.scale, marker.genes.phase,'GeneID')
saveRDS(sc.rna.mu.scale, '../Input/toxo_cdc/rds/sc_rna_spline_mu_scale_transition_clusters.rds')

gene.clusters <- sc.rna.mu.scale %>% dplyr::select(GeneID, cluster.atac, cluster.rna) %>% 
  distinct() 
gene.clusters <- left_join(gene.clusters, marker.genes.phase, by = 'GeneID')

gene.clusters$GeneID <- gsub('-', '_', gene.clusters$GeneID)
gene.clusters <- left_join(gene.clusters, prod.desc, by = c('GeneID' = 'TGME49'))

write.xlsx(gene.clusters , '../Output/toxo_cdc/tabels/gene_clusters.xlsx')

p1 <- ggplot(sc.rna.mu.scale, aes(x = x, y = reorder_within(GeneID, -peak.ord, cluster.rna), fill = expr)) + 
  geom_tile() + 
...
p2 <- ggplot(sc.rna.mu.scale, aes(x = x, y = reorder_within(GeneID, -peak.ord, cluster.atac), fill = expr)) + 
  geom_tile() + 
...
p3 <- ggplot(sc.rna.mu.scale, aes(x = x, y = reorder_within(GeneID, -peak.ord, phase), fill = expr)) + 
  geom_tile() + 
  facet_grid(phase~., scales = "free",  space='free',
             labeller=label_wrap_gen(multi_line = TRUE))+
  ylab("Genes") + xlab("time/cells") + ggtitle('scRNA') + 
...
sc.rna.genes.expr.pt <- sc.rna.genes.expr.pt %>% 
  mutate(cluster.rna = case_when(pt.shifted.scaled <  L.trans.rna$transition.points$y[1] ~ "RC1",
                   pt.shifted.scaled >= L.trans.rna$transition.points$y[1] & 
                     pt.shifted.scaled < L.trans.rna$transition.points$y[2] ~ "RC2",
                   pt.shifted.scaled >= L.trans.rna$transition.points$y[2] & 
                     pt.shifted.scaled < L.trans.rna$transition.points$y[3] ~ "RC3",
                  pt.shifted.scaled >= L.trans.rna$transition.points$y[3]  ~ "RC4"),
         cluster.atac = case_when(pt.shifted.scaled <  L.trans.atac$transition.points$y[1] ~ "AC1",
                                  pt.shifted.scaled >= L.trans.atac$transition.points$y[1] & 
                                    pt.shifted.scaled < L.trans.atac$transition.points$y[2] ~ "AC2",
                                  pt.shifted.scaled >= L.trans.atac$transition.points$y[2] & 
                                    pt.shifted.scaled < L.trans.atac$transition.points$y[3] ~ "AC3",
                                  pt.shifted.scaled >= L.trans.atac$transition.points$y[3] & 
                                    pt.shifted.scaled < L.trans.atac$transition.points$y[4] ~ "AC4",
                                  pt.shifted.scaled >= L.trans.atac$transition.points$y[4] ~ "AC5"))

sc.rna.genes.expr.pt$cluster.rna <- factor(sc.rna.genes.expr.pt$cluster.rna, 
                                                     levels = sort(unique(sc.rna.genes.expr.pt$cluster.rna)))
sc.rna.genes.expr.pt$cluster.atac <- factor(sc.rna.genes.expr.pt$cluster.atac, 
                                                      levels = sort(unique(sc.rna.genes.expr.pt$cluster.atac)))
...
saveRDS(sc.rna.genes.expr.pt, '../Input/toxo_cdc/rds/sc_rna_genes_expr_pt_clusters.rds')
...
rna.trans.d.meta <- sc.rna.genes.expr.pt %>% 
  dplyr::select(Sample, pt.shifted.scaled, cluster.rna, cluster.atac) %>% distinct() %>%
  data.frame()
rownames(rna.trans.d.meta) <- rna.trans.d.meta$Sample

# atac.trans.d.meta <- sc.atac.genes.expr.pt %>% 
...
DimPlot(rna_sub, reduction = 'pca')


Idents(rna_sub) <- 'cluster.rna'
DimPlot(rna_sub, reduction = 'pca')

Idents(rna_sub) <- 'cluster.atac'
DimPlot(rna_sub, reduction = 'pca')
